...
export default function Bookings() {
  ...
  const stats = {
    ...
    actionRequired: inspections.filter(
      (i) => !!(i.actionRequired ?? i.action_required)
    ).length,
  };
  ...
  async function fetchHotels(signal) {
    try {
      ...
      if (normalized.length === 1 && !formData.propertyId) {
        setFormData((f) => ({
          ...f,
          propertyId: normalized[0].id,
          propertyName: normalized[0].name,
        }));
  ...
  function handleServiceUserChange(e) {
    ...
    setFormData((prev) => ({
      ...prev,
      serviceUserId: suId,
      serviceUserName: su ? su.first_name : "",
    }));
  }
  ...
  async function handleSubmit(e) {
    ...
    if (
      ...
      setError(
        "Please fill required fields: Inspection Type, Property, Inspector Name, Inspection Date."
      );
      return;
    }
    ...
    const payload = {
      ...
      issuesFound: Number.isFinite(Number(formData.issuesFound))
        ? Number(formData.issuesFound)
        : 0,
      issues_found: Number.isFinite(Number(formData.issuesFound))
        ? Number(formData.issuesFound)
        : 0,
    ...
    try {
      ...
      if (result) {
        setInspections((prev) => {
          if (editingId) {
            return prev.map((p) =>
              String(p.id) === String(editingId) ? result : p
            );
          }
          return [result, ...prev];
        });
  ...
  const handleEdit = (id) => {
    const item = inspections.find(
      (i) => String(i.id) === String(id) || i.reference === id
    );
    if (!item) return;
    ...
    setFormData({
      ...
      propertyName:
        item.propertyName ?? item.property_name ?? item.property ?? "",
      serviceUserId:
        item.serviceUserId ?? item.service_user_id ?? item.service_user ?? "",
      serviceUserName:
        item.serviceUser ?? item.service_user ?? item.serviceUserName ?? "",
      ...
      <div className="max-w-[1600px] mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <div className="flex items-center gap-2 text-sm text-gray-400 mt-1">
              <span>Property</span> /{" "}
              <span className="text-slate-600 font-medium">Bookings List</span>
            </div>
          </div>
          <button
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <div className="bg-white rounded-xl p-6 shadow-sm flex items-center gap-5">
            <div className="w-14 h-14 rounded-full flex items-center justify-center text-white bg-emerald-500 shadow-emerald-100">
              <IconUsers />
            </div>
            <div>
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm flex items-center gap-5">
            <div className="w-14 h-14 rounded-full flex items-center justify-center text-white bg-[#e77a40] shadow-orange-100">
              <IconPending />
            </div>
            <div>
              <div className="text-sm font-medium text-slate-500">
        ...
        <div className="bg-white rounded-lg shadow-sm border border-slate-200">
          <div className="p-5 border-b border-slate-100 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <button
                onClick={openNewInspection}
                className="bg-[#e77a40] hover:bg-[#d66a30] text-white px-4 py-2 rounded shadow-sm text-sm font-medium flex items-center gap-2 transition-colors"
              >
                Create Booking
              </button>
            </div>
              ...
              <thead>
                <tr className="border-b border-slate-100 text-xs font-bold text-slate-400 uppercase tracking-wider">
                  <th className="p-5 w-10">
                    <input
                      type="checkbox"
                      className="rounded border-slate-300 accent-orange-500 cursor-pointer"
                    />
                  </th>
                ...
                {loading ? (
                  ...
                  inspections.map((r) => (
                    ...
                    >
                      ...
                      <td className="p-5 align-top pt-6">
                        <span className="bg-orange-50 text-orange-500 border border-orange-100 px-3 py-1 rounded-full text-[11px] font-bold tracking-wide">
                          Bookings
                        </span>
                      </td>
                      ...
                      <td className="p-5 align-top pt-5">
                        <div className="font-bold text-slate-700 text-base">
                          {r.inspectionType || r.inspection_type}
                        </div>
                        <div className="text-slate-400 text-xs mt-1 truncate max-w-xs">
                          {r.findings}
                        </div>
                      </td>
                      <td className="p-5 align-top pt-6">
                        <div className="flex items-center gap-2">
                          <span
                            className={`text-xl leading-none ${getPriorityColor(
                              r.priority
                            )}`}
                          >
                            •
                      ...
                      <td className="p-5 align-top pt-6">
                        <div className="flex items-center gap-2">
                          <span
                            className={`text-xl leading-none ${getStatusColor(
                              r.status
                            )}`}
                          >
                            •
                      ...
                      <td className="p-5 align-top pt-5">
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 rounded-full bg-blue-500 flex items-center justify-center text-white text-xs font-bold">
                            {((r.inspectorName || r.inspector_name) ?? "")
                              .substring(0, 2)
                              .toUpperCase()}
                          </div>
                  ...
                  <tr>
                    <td
                      ...
                      className="p-12 text-center text-slate-400 italic"
                    >
                      No inspections found. Click "Create Inspection" to add
                      data.
                    </td>
                  </tr>
                )}
            ...
            <div className="flex justify-between items-start p-6 border-b border-slate-100">
              <div>
                ...
                <p className="text-sm text-slate-500 mt-1">
                  Register a new arrival. This will create a booking and service
                  user record.
                </p>
              </div>
              <button
            ...
            <form onSubmit={handleSubmit} className="p-6 md:p-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="John"
                    required
                    className="w-full rounded-md border border-slate-300 px-3 py-2.5 text-sm focus:border-orange-500 focus:ring-1 focus:ring-orange-500 outline-none"
                  />
                </div>
                ...
                <div>
                  ...
                  <select
                    name="property"
                    required
                    className="w-full rounded-md border border-slate-300 px-3 py-2.5 text-sm text-slate-600 focus:border-orange-500 focus:ring-1 focus:ring-orange-500 outline-none"
                  >
                ...
                <div className="md:col-span-2">
                  ...
                  <textarea
                    ...
                    className="w-full rounded-md border border-slate-300 px-3 py-2.5 text-sm resize-none focus:border-orange-500 focus:ring-1 focus:ring-orange-500 outline-none"
                  />
                </div>
              ...
              <div className="mt-8 flex justify-end gap-3 border-t pt-4">
                <button
                  ...
                  className="px-5 py-2.5 rounded-md border border-slate-300 text-sm font-medium text-slate-700 hover:bg-slate-50"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  className="px-5 py-2.5 rounded-md bg-orange-500 text-sm font-medium text-white hover:bg-orange-600"
                >
                  Create Booking
                </button>
              </div>
