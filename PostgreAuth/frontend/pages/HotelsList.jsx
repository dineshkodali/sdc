...
export default function HotelsList({ user: userProp }) {
  ...
  const fetchHotels = async () => {
    ...
    try {
      ...
      const list = Array.isArray(res.data?.hotels) ? res.data.hotels : (Array.isArray(res.data) ? res.data : (res.data?.data ?? []));
      setHotels(list);
    } catch (err) {
  ...
  const openDetails = (hotel) => {
    const property = {
      ...
      address: hotel.address || `${hotel.city || ""} ${hotel.postcode || ""}`.trim(),
      tags: [
        hotel.property_type || (hotel.is_self_contained ? "Self-Contained" : "Hotel"),
        ...
        <div className="max-w-8xl mx-auto p-6">
          ...
          <div className="flex items-center justify-between mb-4">
            <button 
              ...
              className="w-10 h-10 flex items-center justify-center bg-white border border-gray-200 rounded-full hover:bg-gray-50 hover:shadow-sm transition-all text-gray-500"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {loading ? (
            ...
            hotels.map((h) => {
               ...
               return (
              <div key={id} className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 relative group hover:shadow-md transition-shadow">
