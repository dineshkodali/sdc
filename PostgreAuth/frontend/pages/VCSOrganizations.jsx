...
export default function VCSOrganizations() {
  // Empty initial data — user will provide new data
  const [rows, setRows] = useState([]);
  const [editingId, setEditingId] = useState(null);

  const stats = [
    { label: 'Total Organisations', value: rows.length, color: 'bg-blue-500', icon: <path d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /> },
    { label: 'Active Partnerships', value: rows.filter((r) => (r.status || '').toLowerCase() === 'open').length, color: 'bg-[#f97316]', icon: <path d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /> },
    { label: 'High Risk Partners', value: rows.filter((r) => (r.status || '').toLowerCase() === 'in progress').length, color: 'bg-purple-600', icon: <path d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /> },
    { label: 'Referrals This Month', value: rows.filter((r) => (r.status || '').toLowerCase() === 'resolved' || (r.status || '').toLowerCase() === 'completed').length, color: 'bg-emerald-500', icon: <path d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /> },
  ];
    ...
    <div className="p-8 bg-gray-50 min-h-screen font-sans text-slate-700">
      <div className="max-w-[1600px] mx-auto">
        ...
        <div className="flex justify-between items-start mb-8">
          <div>
            <h1 className="text-3xl font-bold text-slate-800">VCS Organisations</h1>
            <div className="flex items-center gap-2 text-sm text-gray-400 mt-1">
               <span className="hover:text-gray-600 cursor-pointer">Escalations</span> 
               <span>/</span> 
               <span className="text-slate-600 font-medium">VCS Organisations</span>
            </div>
          </div>
          <button 
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
            <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl overflow-hidden">
              <div className="flex justify-between items-start p-6 border-b border-slate-100">
                <div>
                  <h2 className="text-xl font-bold text-slate-800">Task</h2>
